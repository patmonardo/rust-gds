    ...
    fn dfs(&mut self, start_node: i32, parent: i32, get_neighbors: &impl Fn(usize) -> Vec<usize>) {
        ...
        while !stack.is_empty() {
            ...
            if event.last_visit {
                ...
                if low_to >= tin_v {
                    self.articulation_points.set(v as usize);
                }
                
                self.children.add_to(v as usize, 1);
            } else {
                ...
                if !self.visited.get(v as usize) {
                    ...
                    self.children.set(v as usize, 0);
        ...
        if self.children.get(start_node as usize) > 1 {
            self.articulation_points.set(start_node as usize);
        } else {
            self.articulation_points.clear(start_node as usize);
        }
    }
}
