//! FloatDistMultLinkScorer - Faithful 1:1 translation from Java GDS
//!
//! Translated from: org.neo4j.gds.algorithms.machinelearning.FloatDistMultLinkScorer
//!
//! DistMult link scorer implementation using float arrays for embeddings.

use crate::types::properties::node::NodePropertyValues;
use super::link_scorer::LinkScorer;

/// Float DistMult link scorer - translated from Java FloatDistMultLinkScorer
/// 
/// DistMult link scorer implementation using float arrays for embeddings.
/// Implements the DistMult scoring function: score = sum(source[i] * relation[i] * target[i])
#[derive(Debug)]
pub struct FloatDistMultLinkScorer {
...
impl LinkScorer for FloatDistMultLinkScorer {
    ...
    fn compute_score(&self, target_node: u64) -> f64 {
        ...
        for i in 0..self.current_candidate_target.len() {
            res += self.current_candidate_target[i] as f64 * target_vector[i] as f64;
        }
