...
impl Collections<f32> for VecFloat {
    ...
    fn set(&mut self, index: usize, value: f32) {
        if index < self.data.len() {
            ...
            self.data.resize(index + 1, 0.0f32);
            self.data[index] = value;
        }
    }
    ...
    fn mean(&self) -> Option<f64> where f32: Into<f64> {
    ...
    fn std_dev(&self) -> Option<f64> where f32: Into<f64> {
        if self.data.len() < 2 {
            None
        } else {
            let mean = self.mean()?;
    ...
    fn variance(&self) -> Option<f64> where f32: Into<f64> {
        if self.data.len() < 2 {
            None
        } else {
            let mean = self.mean()?;
