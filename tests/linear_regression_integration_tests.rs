...
use rust_gds::ml::models::{Features, RegressorTrainer, Regressor, ModelData, TrainerConfig};
...
mod basic_training {
    ...
    fn test_simple_linear_relationship() {
        ...
        assert!((prediction_1 - 3.0).abs() < 0.1, "Prediction 1: expected ~3.0, got {}", prediction_1);
        assert!((prediction_2 - 5.0).abs() < 0.1, "Prediction 2: expected ~5.0, got {}", prediction_2);
        assert!((prediction_3 - 7.0).abs() < 0.1, "Prediction 3: expected ~7.0, got {}", prediction_3);
        assert!((prediction_4 - 9.0).abs() < 0.1, "Prediction 4: expected ~9.0, got {}", prediction_4);
    }
    ...
    fn test_multi_dimensional_features() {
        ...
        assert!((prediction_1 - 6.0).abs() < 0.2, "Prediction 1: expected ~6.0, got {}", prediction_1);
        assert!((prediction_2 - 8.0).abs() < 0.2, "Prediction 2: expected ~8.0, got {}", prediction_2);
        assert!((prediction_3 - 9.0).abs() < 0.2, "Prediction 3: expected ~9.0, got {}", prediction_3);
        assert!((prediction_4 - 11.0).abs() < 0.2, "Prediction 4: expected ~11.0, got {}", prediction_4);
    }
}
...
mod regularization_tests {
    ...
    fn test_l2_regularization_effect() {
        ...
        let trainer_no_reg = LinearRegressionTrainer::new(1, train_config_no_reg, termination_flag.clone());
        ...
        assert!((pred_no_reg - 2.0).abs() < 0.5, "No regularization prediction: expected ~2.0, got {}", pred_no_reg);
        assert!((pred_reg - 2.0).abs() < 0.5, "Regularized prediction: expected ~2.0, got {}", pred_reg);
        ...
        assert_ne!(pred_no_reg, pred_reg, "Regularization should affect predictions");
    }
}
...
mod model_persistence {
    ...
    fn test_model_serialization_and_deserialization() {
        ...
        let linear_regressor = regressor.as_any().downcast_ref::<LinearRegressor>().unwrap();
        ...
        let deserialized_data = LinearRegressionData::from_bytes(&serialized)
        ...
        assert!((original_prediction - deserialized_prediction).abs() < 1e-10, 
            "Predictions should match after serialization/deserialization: {} vs {}", 
            original_prediction, deserialized_prediction);
    }
}
...
mod configuration_tests {
    ...
    fn test_training_configuration() {
        ...
        assert_eq!(train_config.method(), rust_gds::ml::models::TrainingMethod::LinearRegression);
        ...
        assert_eq!(default_config.method(), rust_gds::ml::models::TrainingMethod::LinearRegression);
    }
    ...
    fn test_configuration_to_map() {
        ...
        assert_eq!(config_map.get("method").unwrap().as_str().unwrap(), "LinearRegression");
        assert_eq!(config_map.get("penalty").unwrap().as_f64().unwrap(), 0.1);
        assert_eq!(config_map.get("batch_size").unwrap().as_u64().unwrap(), 4);
        assert_eq!(config_map.get("learning_rate").unwrap().as_f64().unwrap(), 0.01);
...
mod edge_cases {
    ...
    fn test_single_sample_training() {
        ...
        assert!((prediction - 2.0).abs() < 1.0, "Single sample prediction should be reasonable: {}", prediction);
    }
    ...
    fn test_zero_features() {
        ...
        assert!((prediction - 1.0).abs() < 0.5, "Zero-feature prediction should be close to target: {}", prediction);
    }
    ...
    fn test_identical_features() {
        ...
        let prediction = regressor.predict(&[1.0]);
        assert!(prediction > 0.0, "Prediction should be positive: {}", prediction);
    }
}
...
mod performance_tests {
    ...
    fn test_large_dataset_training() {
        ...
        assert!((prediction_0 - 1.0).abs() < 0.5, "Prediction at x=0 should be close to 1.0: {}", prediction_0);
        assert!((prediction_5 - 2.0).abs() < 0.5, "Prediction at x=0.5 should be close to 2.0: {}", prediction_5);
        assert!((prediction_10 - 3.0).abs() < 0.5, "Prediction at x=1.0 should be close to 3.0: {}", prediction_10);
    }
}
