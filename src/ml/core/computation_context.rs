...
impl ComputationContext {
    ...
    pub fn forward(&mut self, variable: &dyn Variable) -> Box<dyn Tensor> {
        ...
        // Check cache
        if let Some(cached) = self.data.get(&var_key) {
        ...
        self.data.insert(var_key, result.clone_box());
        result
    }
