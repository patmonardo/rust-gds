...
impl DefaultValue {
            ...
            Some(v) => match v {
                JsonValue::Number(n) => {
                    if let Some(i) = n.as_i64() {
                        ...
                        exact_double_to_long(f).map_err(|e| DefaultValueError::UnsafeConversion(e))
                    } else {
    ...
    pub fn float_value(&self) -> Result<f32, DefaultValueError> {
        ...
        not_overflowing_double_to_float(d).map_err(|e| DefaultValueError::UnsafeConversion(e))
    }
    ...
    pub fn double_array_value(&self) -> Result<Option<Vec<f64>>, DefaultValueError> {
        match &self.value {
            ...
            Some(JsonValue::Array(_)) => Ok(None),
                ...
                for d in vec {
                    out.push(
                        not_overflowing_double_to_float(d)
                            .map_err(|e| DefaultValueError::UnsafeConversion(e))?,
                    );
                }
                Ok(Some(out))
            }
...
fn exact_double_to_long(d: f64) -> Result<i64, String> {
    if d.fract() == 0.0 {
        let as_i = d as i64;
...
fn exact_long_to_double_if_int(
    n: &serde_json::Number,
    fallback: f64,
...
fn exact_long_to_float(l: i64) -> Result<f32, String> {
    let limit = 1i128 << 24;
    let l_i128 = l as i128;
    if l_i128 >= -(limit as i128) && l_i128 <= (limit as i128) {
        Ok(l as f32)
...
fn not_overflowing_double_to_float(d: f64) -> Result<f32, String> {
    const FLOAT_MAX_VALUE: f64 = 3.4028234663852886e38_f64;
    if d.is_nan() {
        Ok(f32::NAN)
    } else if d > FLOAT_MAX_VALUE || d < -FLOAT_MAX_VALUE {
...
pub static DOUBLE_ARRAY_DEFAULT_FALLBACK: Lazy<Option<Vec<f64>>> =
    Lazy::new(|| DEFAULT.double_array_value().ok().flatten());
pub static FLOAT_ARRAY_DEFAULT_FALLBACK: Lazy<Option<Vec<f32>>> =
    Lazy::new(|| DEFAULT.float_array_value().ok().flatten());
pub static LONG_ARRAY_DEFAULT_FALLBACK: Lazy<Option<Vec<i64>>> =
    Lazy::new(|| DEFAULT.long_array_value().ok().flatten());
